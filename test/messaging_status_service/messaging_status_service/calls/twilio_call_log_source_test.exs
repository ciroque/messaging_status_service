defmodule TwilioCallLogSourceTest do
  ...
  test "CallSid lookup results in 404" do
    ...
    assert {:error, :not_found} == TwilioCallLogSource.retrieve_call_log(@expected_call_sid)
  end
  ...
  test "call log with canceled status yields completed" do
    ...
    assert {:ok, :completed, body} == TwilioCallLogSource.retrieve_call_log(@expected_call_sid)
  end
  ...
  test "call log with queued status yields in_progress" do
    ...
    call_log = {:ok, %{status_code: 200, body: encode(body)}}
    HttpClientMock |> expect(:get, fn _, _, _ -> call_log end)
    assert {:ok, :in_progress, body} == TwilioCallLogSource.retrieve_call_log(@expected_call_sid)
  end
  ...
  test "400 response" do
    ...
    HttpClientMock |> expect(:get, fn _, _, _ -> call_log end)
    assert {:error, body} == TwilioCallLogSource.retrieve_call_log(@expected_call_sid)
  end
  ...
  test "call errors, network segment or something" do
    ...
    assert {:error, response} == TwilioCallLogSource.retrieve_call_log(@expected_call_sid)
  end
